...
def _norm_args(args):
    return {k: _norm(v) for k, v in sorted((args or {}).items())}
...
def _args_match(expected, actual):
    ...
    for k, ev in (expected or {}).items():
        if _is_dynref(ev):
            continue
        if k not in actual or _norm(ev) != _norm(actual.get(k)):
            return False
    return True
...
def fold(calls):
    ...
    for c in calls:
        ...
        args = c.get("args", {}) or {}
        if fn in WRITE_KEY:
            key = _norm(args.get(WRITE_KEY[fn], ""))
            state[(fn, key)] = _norm_args(args)
        elif fn in READ_TOOLS:
            read_finals[fn] = _norm_args(args)   # last call per read fn wins
        else:  # unknown tool: treat as write keyed by fn (conservative)
            state[(fn, "")] = _norm_args(args)
    return state, read_finals


def state_track_pass(expected_calls, actual_calls):
    es, er = fold(expected_calls)
    as_, ar = fold(actual_calls)
    diffs = []
    ...
    for k in es:
        if not _args_match(es[k], as_.get(k)):
            ...
            fn = k[0]
            if _is_dynref(k[1]):
                if any(k2[0] == fn and _args_match(es[k], as_[k2]) for k2 in as_):
                    continue
    ...
    for fn in er:
        ...
        elif not _args_match(er[fn], ar[fn]):
...
def main():
    ...
    for folder in folders:
        ...
        rows.append({"id": sid, "folder": folder.name, "state_pass": ok,
                     "exact_pass": official, "diffs": diffs})
    n = len(rows)
    print(f"state-track | provider={args.provider} | {n_pass}/{n} pass "
          f"({100.0*n_pass/max(n,1):.1f}%)")
    exact_n = sum(1 for r in rows if r["exact_pass"])
    print(f"official exact (recomputed per folder): {exact_n}/{n}")
    print(f"calibration (state, exact): {mat}   "
