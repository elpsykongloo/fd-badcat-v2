...
- **打断提示词**：demo 不再覆盖 `interrupt`，直接继承 `src/config.yaml` 的原始 HumDial 全文（含 8 条示例），不附加或改写规则。原版在已定位的两个含糊录音片段上返回 `continue`，已撤回的精简版返回 `switch`；这只是个案对照，不是整体误打断率或回声消除验收。证据不足时的语义拒识、停止播报与新回答生成解耦仍未实现。
...
## 9. 误打断边界与后续设计（分析方案，尚未实施）

本次实际变更仅为恢复原版 `interrupt`，不把下述方案当成已上线能力。原版/精简版本机同音频对照：两个含糊片段均 `continue`/`switch`，真实语言切换请求均 `switch`；两个含糊片段复测一致。原版只删示例仍均 `continue`，精简版补回原示例及结尾仅纠正其中一个，故不能将退化单独归因于删示例。仅记录聚合结果，不提交真实用户录音、转写或会话日志。原版同样预设用户发声，也没有明确的噪声/回声/不确定兜底，恢复它不等于解决声学输入问题。
...
分开“输入是否可信”“是否让出话轮”“是否有可回应的完整内容”三个问题；模型输出只是候选决策，引擎负责执行条件。保留本次原版作为稳定基线，新策略须有独立版本/开关和针对性音频对照，不再直接精简改写原版。
...
可将最后三种情况表达为 `STOP_ONLY / YIELD_WAIT / YIELD_READY`，加 `KEEP` 构成专用、严格校验的枚举，而不是借用 TACT 工具 JSON 解析器。名称/协议还不是实现约定。只有 `YIELD_READY` 且输入版本仍有效才有资格发布回答。若继续保留原二分类作为第一阶段，`switch` 后必须再经过此路由；原 `judge` 只能辅助判断是否说完，不能判断“停”是否需要回答。后续若合并成一次多轴模型调用以减少请求，须作为新提示词版本单独验证；两个相关模型判定相同也不构成独立声学证明。

保守动作依阶段而异：未停播时判定失败保持播放；已确认停播后，后续判定失败保持安静监听，不能恢复旧音频或用默认道歉重新开口。持续语音可作为争取话轮的证据，但只计已确认的有效近端语音，不能把持续 VAD/时间到期当作充分证据。不能一刀切删除所有短音，否则“停”同样会失效。

### 输入、候选与播放需要配套修改

1. **输入证据**：先补浏览器实际音轨处理设置、设备采样率、播放/取消采样位置及 VAD 区间的观测；播放参考 PCM/对齐信息要有界并与麦克风同源时钟对应，不能直接相减服务端/客户端时钟。回声处理需要实际声学证据，不能仅凭 `echoCancellation: true`、转写非空、音量或模型自报置信度放行；播放文本只作被标注的参考，不是新用户指令，更不能用文本相同直接删掉用户重复。现有日志不足以宣称已能区分回声与真人。
2. **单一准入入口**：短打断、长打断、开播前续说、播放 EOF 后的片段移交共享片段身份与准入结果。原始 VAD 事件先形成待核验输入，不直接等同已确认的用户新轮次。开播前可暂缓候选发布，确认真实续说才整链作废；判为无效输入则保留候选。需分别管理原始检测版本和已接纳输入版本，避免误 VAD 先递增 epoch 把旧候选永久作废。
3. **让出话轮不调用 response**：确认打断时，由 actor 单写者一次性停止旧流、撤销其 HTTP/队列、转监听并保留插话音频及判定期间新收到的帧。独立的 `maybe_start_response` 仅消费已接纳且可回应的输入；拒绝片段不可通过迟到 ASR、轮次收尾或等待超时重新进入历史/候选。
4. **等待要有原因**：区分普通未完句、显式等待、纯停止和无效输入。原有 2.5 秒超时不能跨原因强制回答；显式等待/停止期间保持静默，闲置私有任务可按预算释放，缓冲有界，但不得凭超时编造问题。对有效但始终不清楚的请求，是否主动澄清是单独策略，不是错误回退。
5. **低延迟不靠提前公开**：输入检查、话轮/可回应性判断可尽量放在原停顿窗口内；新文本与首句 TTS 仍可私有投机，但不能因为取消旧流就获得发布资格。复用 candidate/utterance ID 和取消能力，并限制普通请求槽。新增检查有开销；需测首响与真实打断延迟，不承诺无成本。
6. **取消与历史**：浏览器确认停止后回报旧 utterance 的实际播放样本位置，拒绝旧 PCM/迟到 EOF 复活；从发出取消到确认停止期间仍考虑残留播放回声，不能立刻假定完全静音。已取消不自动续播；未来若支持暂停/续播，要另建保留 PCM 与游标的协议，现有 `cancel()` 已销毁播放节点。历史分别保留生成文本、被打断标记及可确认播完的句子范围；没有音文对齐就不能宣称精确“听到前缀”。

### 最小验收范围

定向覆盖：非语音/回声/附和不取消；短“停”能取消且不生成；“等一下”超过 2.5 秒仍安静；未完纠正续说后只答一次；完整新请求正常回答；无效长 VAD 不在 1.5 秒强停；拒绝片段跨 EOF/迟到 ASR 不复活；取消与开播/完成回执竞态不串轮。另以真实笔记本耳机与外放分别检查输入闭环，报告误打断次数、真实打断漏检、停止延迟与首响延迟。纯状态机/虚拟麦克风测试不能代替声学验收，不需要为此运行 RB 全量评测。
