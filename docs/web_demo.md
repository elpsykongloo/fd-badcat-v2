...
线上控制协议为 `transcript-first-v1`：单次 Omni 请求先输出转写，再输出小写四态标签，例如 `{"transcript":"停","label":"stop_only"}`。只接受依次为 transcript/label 的两个字符串字段，拒绝重复键、额外字段、逆序、截断、空转写配动作及非四态标签；完整结果校验前不执行动作。正常一次请求，异常仍最多修复一次，共用 2 秒总预算（含容量等待），最终回退 keep；修复不回喂上次无效转写。转写仅进入受限审计，不替代聊天 ASR/历史，也不是独立证据或声源证明。不串接独立 ASR、第二个分类器或投票框架；停止/等待/回答分权、投机和取消栅栏不变。原 HumDial interrupt 提示词与关 guarded_turns 的基线保持不动。
...
`--current-prompt` 对路由案例会同时重建当前协议消息、删除旧参考文字；默认回放仍使用原请求和相应版本解析器，不能用新 JSON 规则误判旧标签归档。
...
当前 demo 只做两处调整：`engine.input_preroll_ms: 320` 保留 VAD 触发前的真实音频（原为160ms）；四态 `input_route` 的 presence/frequency penalty 均为0。VAD阈值仍为0.5、结束静音仍为100ms，640ms确认/1.5s长输入检查/2.5s旧基线参数、四态prompt和停止/回答分权均不变。取已缓存的历史音频不增加一个160ms等待窗，但输入更长会增加少量编码/推理成本，并可能影响模型读出；不是行为等价或无成本保证。
