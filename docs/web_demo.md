...
未开启 §9 时历史仍保存完整生成文本；开启后，取消回复只把浏览器 ACK 覆盖的完整句子写入模型历史，打断状态留在内部记录和 trace，不编码成 assistant 自然语言；若没有完整句子则不建立该轮 assistant 历史。这不是词级“用户已听到前缀”。预热不是持续健康检查，也不能消除每次会话的所有初始化耗时。提示词不是事实性保证：自造天气问题的功能烟测仍出现无依据的天气断言，不能据此宣称聊天质量已验收。这些变更没有修改 RB/TACT 评测配置或建立新的正式成绩。
...
音频路由协议为 `transcript-first-v1`：单次 Omni 请求先输出转写，再输出小写四态标签，例如 `{"transcript":"停","label":"stop_only"}`。只接受依次为 transcript/label 的两个字符串字段，拒绝重复键、额外字段、逆序、截断、空转写配动作及非四态标签；完整结果校验前不执行动作。音频阶段正常一次请求，异常最多修复一次；修复不回喂上次无效转写。2026-09-16 增加下述条件性 `played-reply-v1` 纯文本复核，与音频阶段共用原 2 秒总预算（含两阶段容量等待），失败保持 keep。音频转写不替代聊天 ASR/历史；现在它还作为条件复核的固定输入，并非独立声学证据或声源证明。不新增 ASR、模型或投票；停止/等待/回答分权、投机和取消栅栏不变。原 HumDial interrupt 提示词与关 guarded_turns 的基线保持不动。
...
5. **停止回执与历史**：浏览器销毁旧排程，按 WebAudio 时钟计入当前包已播部分，报告 `playback_stopped`；未来包不计已播。服务端校验 ID/样本范围，仅已取消流可更新。按句末音频样本边界只保留可确认完整播完的句子；无完整句时删除该轮 assistant 历史。打断状态只留在内部记录和 trace，不向模型注入自然语言标记，避免后续回答复述控制元数据；完整生成文本留审计史/页面。不是词级对齐或物理听到证明；新候选的固定历史快照不被晚回执改写。
6. **消息与观测**：`route_messages()` 只发送 assistant_playing 布尔元数据及麦克风音频，**不发送助手参考文字**（包括空字段），防止转写把参考抄成用户语音。预热、线上路由和 canary 共用此构造器。下方增量参考仍内部维护、冻结和审计，双声道 PCM 参考及声学回声过滤保留。案例上下文记录 route_protocol/reference_sent_to_model；`/api/demo/info` 暴露协议和 route_reference_text=false。trace 记录 input_dispatch/decision/admitted/rejected/stale、声学证据和停止回执，UI 的等待/忽略逻辑不变。启动回读偶发不符最多可见重试一次，仍须严格归一相等；连续失败拒绝启动。
...
窗口包含完整当前句，可能带有该句尚未听到的后半段；ACK 有延迟，参考还固定于输入开始时，不是逐字实际听到前缀。它不是回声证明或声纹。原 speech-reference-v1 发布时未改 prompt；现在 transcript-first-v1 不再把此窗口放入分类模型请求，但仍保留其内部诊断和隔离机制。模型不带参考文字的收益/代价需成对回放，不宣称回声零误判。

### 已播上下文短答复核（played-reply-v1）

本机制替代未上线的“参考末尾问号”方案E。配置入口是 demo 独立 `prompts.input_reply`；删除该项可恢复单阶段路由。音频路由请求仍不带助手原文；只有闭合、播放期、有效非空转写且初判 keep，并有输入开始时已播完上下文，才额外调用同一个 Omni 做一次纯文本复核。它判断助手是否确实向当前用户提出尚待回答的问题/选择/许可请求，以及用户是否在回答；反问、自问自答、故事人物之间的问题和普通附和保持 keep。无问号的请求按语义处理，生产代码没有问号门或短答词表。

上下文复用 `sentence_end.end_sample` 与校验过的 `played_samples`，取输入起点已确认播完的最后最多3句，总长≤512字符，不从句中间裁切。使用独立于 `playback_sentence_window` 的 `InputSpan.reply_context`，连空快照也只冻结一次，后续 END、ACK、开播竞态和 EOF 不补入未来文本；reset/续说沿用输入 ID/revision 栅栏撤销在飞复核。当前未播完句不进入文本复核，故抢在问题播完前的含糊短答仍可能 keep；这是明确接受的保守边界，不引入词级对齐。回执证明软件播放进度，不证明用户物理听见。

文本复核严格只接受 `keep` 或 `yield_ready`，不改写转写、不允许 stop/wait，不做格式修复。网络错、超时、格式错均保持原 keep；初判 stop/wait/ready、空转写、未闭合输入、无完整上下文均不增加调用。复核期间旧播放继续；只有最终有效 ready 才经原 Actor 接纳、取消和回答流程。极端情况下音频阶段一次修复再复核共3次请求，但共享原总预算，不单独延长2秒。

预热新增真实文本提问/叙述两项 canary；案例库以 `input_route` 和 `input_reply` 分别记录实际请求，使用同一 input ID/revision 关联，复核记录中包含冻结文本但没有音频。`reply_protocol`、`reply_context_chars`、`reply_played_samples` 和 `decision_stage` 可用于审计。API 的 `route_reference_text=false` 指音频阶段；`reply_context=completed_sentences_at_input_onset` 指文本复核。原请求重放默认不变，`--current-prompt` 的 `input_reply` 只换复核 prompt/零惩罚，保留冻结文本；单条音频案例重放不会自动补跑复核。

验收入口：`scripts/check_input_reply.py`（自造语义对照）、`scripts/check_input_reply_live.py`（真实浏览器链路）、`tests/test_input_reply.py`（预算、快照和取消竞态）。聚合结果见 `exp/web_demo/played_reply_validation.json`。344个输入变体的串行回归共583次请求，126条复用真实录音配构造问答背景从0→123，24条叙述及24条反问对照保持正确；原场景60附和59→59、40停止/等待40→40、40正常请求38→38、20第三方4→4，8非语音均keep。无错/修复/超时/回退，239次复核本机中位73.146ms。它们不是未见真人会话或官方成绩，第三方归属仍有明显缺口。

两段历史“都可以”按实际ACK重建：第一段开口时问句尚未播完，保守keep；第二段已播完问句，keep→ready。私有回归原计划沿用旧实验的两段ready，回读播放边界后、观察这两段输出前单独写入期望勘误，原计划/行均保留。语义开发曾出现反问/引用/自答误接，收据完整保留；最终冻结后另16个中英自造文本对照全部通过。小样本通过不保证所有修辞形式均正确，文本复核也不能消除音频阶段自身幻觉。
...
`scripts/evaluate_actor_routes.py` 提供串行旧/新配对调用（`--suite humdial|humdial-prefix|cases|synthetic`），需要显式提供旧基线计划 `--baseline-plan` 和新输出目录 `--output`。HumDial 使用已获授权的本地训练集：按语言×场景固定抽样，类别不直接充当四态答案，终止类按表达意图事先复核；第三方声源归属与有歧义片段单列。只落盘样本定位、判定标签、通过计数及耗时，不落盘其音频、参考/模型转写。数据不随仓库分发，遵守 Hum-Dial Challenge 的非商业研究使用约束。私有真实案例的标签必须人工确认；未标注案例只报告变化，不计准确率。

2026-09-08 验收：595 个输入变体共 1190 次配对模型调用，包含 320 个 HumDial 原片段、其中 34 个停顿前缀、41 个归档快照和 200 个合成/非语音变体；不是 595 个独立真人样本或官方基准分数。HumDial 主语义 278 条旧/新 243/267（新增正确 25、退化 1），72 个非语音上下文探针均 keep；不完整前缀 31 条安全等待从 5 提高到 19，仍有 12 条过早 ready。第三方语音 40 条仅 6/8 条被拒，声源识别没有解决。真实短问句已接住，但已知短促停止片段仍漏判；英文附和另有一例转写误读导致误接纳。320 条 HumDial 的本机单路由 p50 约 115→299ms，不能宣称投机完全抵消增加的耗时。以上为受控 demo 的净改善验收，不是零退化或生产声学鲁棒性保证。聚合收据：`exp/web_demo/route_transcript_validation.json`。

## 11. 短输入边界与路由解码（2026-09-08，未新增停止判据）

本节记录的2026-09-08调参只做两处调整：`engine.input_preroll_ms: 320` 保留 VAD 触发前的真实音频（原为160ms）；四态 `input_route` 的 presence/frequency penalty 均为0。VAD阈值仍为0.5、结束静音仍为100ms，640ms确认/1.5s长输入检查/2.5s旧基线参数、四态prompt和停止/回答分权均不变。取已缓存的历史音频不增加一个160ms等待窗，但输入更长会增加少量编码/推理成本，并可能影响模型读出；不是行为等价或无成本保证。2026-09-16新增的同模型文本复核也使用零惩罚，另见§9。

采集仍使用既有有界deque，不增加第二套VAD/ASR/关键词停止通道。只有协商后的guarded Actor读取新前置保留值，legacy/RB/TACT保持原路径；配置允许0–960ms、16ms整数倍。缺省160ms，改回160并重启即可单独回退这项。新案例记录实际`preroll_samples`与`input_timing`，刚连接时实际已收到的前缀可能不足320ms；不补零伪称拥有真实前缀。它仍是模型输入归档，不是完整麦克风会话录音，旧档案丢失的前段不能补回。

路由通过`qwen_text_payload(..., route=True)`集中取消两种生成惩罚；实际SSE请求、同步启动预热、canary、案例归档同构，修复请求也使用同样参数。聊天/shift/原binary/TACT生成参数不变，保存的旧案例默认继续逐请求原样回放。显式`demo_cases.py replay --current-prompt`切到当前路由时，同步替换这两项惩罚，其他原采样参数保留，报告会标明；不会偷偷改写旧case文件。`/api/demo/info`可核对`input_timing`与`route_penalties`，仍只报告配置，不承诺GPU健康。

**选参证据与限制**：27组固定边界候选（阈值.5/.4/.35 × 前置160/320/480ms × 结束静音100/160/224ms）；520个HumDial训练源片段按语言/场景内录音编号组分170筛选、350确认，排除前轮计划中的1760个源ID。完整标注用户段（另取80ms实录上下文）前后加640ms静音，以原音量及0.25倍音量回放；13种自造非语音仅作额外VAD压力，不是自然外放噪声。CPU结果是能量/分段指标，不能当词准确率；没有新保存HumDial音频或源/模型转写。

实际Omni筛选77条边界压力输入，旧参数/只320ms/阈值.4+320ms/再加160ms结束静音，动作口径分别61/64/62/63正确，故冻结只改前置保留。确认350源的低音量开头丢失>2%能量从14→4（包含双方均未触发的1条），原音量2→1；VAD触发与结束时点逐项不变。208条确认路由189→191，gain3/loss1，完整转写归一相等168→167；80条停止/等待78→78、80条附和78→78，**没有证明短停止召回提升或零退化**。这组模型比较固定零惩罚、用250ms模拟在飞续说合并后取首个闭合输入，不能冒充完整引擎或物理笔记本延迟成绩。主比较689次实际请求（筛选274、确认415，直接相同输入复用不重复计）；首次服务未运行的连接失败批次留档为无效，不计模型效果。私有计划、标签、计数及失败记录在`exp/demo_cases/diagnostics/vad_tuning_20260908/`，自造浏览器烟测与聚合收据另见`exp/web_demo/input_tuning*`。

**结论边界**：新录音将保留更完整的前缀，不等于确认上一例必由VAD截断。旧短停可被零惩罚救回，但最新几个短片段在纯转写/补零/改提示词下仍未识别成停止词；不能据此把噪声、含糊音节或ASR非空直接提升为停止权。后续若仍漏，优先真实笔记本有界连续音频/耳机条件对照，再区分采集预处理、边界及模型读出，不继续盲降VAD阈值。
...
以下较复杂分支只作历史备选；当前依奥卡姆剃刀未接入，也不将其作为本轮修复承诺：
...
**不存在一般性的“增加停止召回且绝不伤及其他输入”保证**：只要让原keep的样本获得停止权，就改变了判定边界。可保证的是工程隔离、输入身份、取消幂等、绝不偷偷派生新回答；效果上的无回退需要有代表性的成对数据支持。若将来另行引入复杂分支，应先影子运行、固定保留集过门、获得用户授权并保留回退，不能顺手改prompt或加关键词强停。
