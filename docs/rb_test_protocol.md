...
- **L7 反窗口夹**：低/中 π 点对固定臂的 learned-only 通过**全部落在 L7**
  （π=.02 的 6/6、C1@π.10 的 4/4）；低 π 点与 δ0 的通过集也非嵌套（±3 互换）。
  机制：L7 补偿场景要求 forward 调用**已经提交**、撤销走补偿路径——窗口未过期时
  cancel 直接吞掉 pending op，state 反而错。这是全基准唯一"提交更快 = 更对"的
  格（44 夹里 ~6 夹），也是固定窗口 L7 .3864 的天花板注脚（16 个 window-helps 夹
  与 6 个 anti-window 夹在同层互斥）——**任何固定 δ 都吃不下 L7 全层**，机制上
  只有 finality/意图感知才能分开这两类；本判决同时钉住"学习头也没分开"（低 π 恰好
  救 6 个 anti-window 夹是排序副产品：done-p50 2.27 的代价是丢 46 个窗口夹）。
...
L7 反窗口夹（7.3）是唯一被实证的"超越固定窗口需要什么"：不是更好的时序风险
排序，而是区分"修订意图 vs 撤销意图"的内容理解——留作 future work 的准确表述。
