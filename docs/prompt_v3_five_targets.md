...
### 3.3 Round-1 判决（2026-07-09 跑后追加；**迭代按预注册封盘**）

- **门全未中**：30 子集 17/30 ✗（<19）；rollback 13/17 ✗（<14）；7 gain 零丢失 ✗（tr19 丢）；
  全量点预测 ✗（TACT 0.650 ≠ 0.660，sblock 0.670 超预测 +1）。
- **全量一次性验证（已花掉）**：TACT d150 0.620→**0.650**（gain eco23/eco25/fin04/tr18/fin19，loss fin23/tr19，净 +3）；
  sblock 0.650→**0.670**（gain eco19/fin19/hou21，loss fin23，净 +2）。三处 diff 的靶**全部命中**（eco23✓ eco25✓ fin19✓），代价两处误伤。
- **误伤根因（零 GPU 逐夹诊断）**：
  - finance_23：**臂中立，双臂同死**（v3 双臂过 → v3.1 双臂挂）。规则 11 的显式撤回例句捕获了规则 13 的
    替换构式（"never mind — just set up autopay"），输出 0 调用；双重惰性条款没压住。
  - travel_19：**TACT 特异**（sblock 仍过）。规则 10 诱发"early launch → 补齐日期后 re-launch"而非 patch
    pending op——两次 `search_flights` 被 pop(0) 位置对齐双杀。规则 14 的日期形态本身正确。
- **哨兵**：ecommerce_13 两夹维持 fail（规则 15 无害化确认；夹2 属既有链式差距，sblock 过 TACT 不过，非 v3.1 效应）。
- **栈一致性核验**：30 子集（音频栈）与全量重跑（text-only stage-0 / QWEN_MAX_MODEL_LEN=8192 / --workers 12）
  在 30 夹上判决**逐位一致（0 分歧）**；全量 TACT 0 cache hit 系载荷编码差异（键含 audio/text block），
  决策 ops 稳定 —— 与 D1.5 drift probe OPS-STABLE 互证。v3.1 各臂延迟字段（含现实档 P-1/P-3/P-4 的 infer 锚）
  均为并发产物，**只作信息位**；P-2=1.500 恒等式锚免疫，可引用。
- **封盘**：按 §3.2 "不中不得再迭代"，**v3.2 不做**。两处误伤的修法已知但不执行
  （规则 11 需加规则 13 让位条款；规则 10 需限定 ALREADY EXECUTED、pending 走 patch）——转为 W4 学习组件的
  训练素材与失败分类学。两轮词条工程实测呈**刀口零和**（修 fin19 → 丢 fin23）：规则工程天花板已量化，
  是学习组件立项的直接动机。
- **口径落定建议**：主表 = v3.1（双臂同 prompt 之下双方最强：0.650 vs 0.670；−2pt 差距逐夹全归因 =
  hou25 TACT 独胜 ↔ eco13₂/eco15/tr19 sblock 独胜，其中 eco15 为 W2 已知脏轨迹夹、tr19 为本轮已归因回归）；
  v2 保留为"无 prompt 工程"参照行；δ 曲线形状（eager 惩罚 + 单调升 + d200→sblock 收敛）三个 prompt 版本复现。
