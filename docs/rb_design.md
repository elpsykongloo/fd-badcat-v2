...
各 category 请求彼此独立，主线路径默认以 `DEEPSEEK_WORKERS=100` 并发（本 bank 实际受
任务数上限），每请求以非敏感、prompt-hash 的 `user_id` 隔离；每任务独立 client/五次硬重试，
全部成功后才按固定 job index 原子写 bank，因此完成顺序或单项失败均不能制造部分 bank 或改变
字节布局。
