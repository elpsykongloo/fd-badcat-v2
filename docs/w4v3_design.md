...
> **状态：v1 FROZEN；Arm C0 已判读且 P4 FAIL 触发 C1；Arm C1 §11.4 已实跑并归档原始收据，P5/C1 版 P4 待用户裁决（2026-07-15）**。结构/判据/探针门于实跑前冻结（第一段）；Phase-1 收据 = §9；数值常量与预测于 §10 冻结为 v1（第二段）；C0/C1 Phase-2 原始收据 = §10.6/§11.5。Arm F/P 经探针判读不启用（§10.1，终局）。
