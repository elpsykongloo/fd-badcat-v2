...
> **状态：v1 FROZEN（2026-07-15）**。结构/判据/探针门于实跑前冻结（第一段）；Phase-1 收据 = §9；**数值常量与预测已在 §10 冻结为 v1（第二段）——Phase-2（Arm C0 π 网格 + 条件 C1）据 §10.4 放行**。Arm F/P 经探针判读不启用（§10.1，终局）。
...
## 6. Phase-2 结构（常量占位，冻结时填数）**→ 数值已在 §10 冻结；本节保留为历史结构，λ_mix 占位被 §10.2 作废（=0）**
...
- **复合 kill：不触发**。事件数 748 ≥ 300；"停顿分布与合成 v2 支撑无显著差"的检验现冻结为：实测 max-pause p50 落入合成 fast 分量 config-median 带 [e^{−1.5}, e^{−0.5}] = [0.223, 0.607] ⇒ 无差。实测 0.900(zh) / 1.092(en) 均在带外（超上界 48%/80%）⇒ 有实质差 ⇒ 第二合取假。W 覆盖核算：W=1.5 覆盖实测停顿 ~p75–p85（zh p75 1.284 < 1.5 < p90 1.658；en p50 1.092 < 1.5 < p75 1.572）；含屏障宽限的有效 2.5s 覆盖 **731/748 = 97.7%**——W_PROTECT=1.5 维持不动的实测依据。
...
- **C0 公式（冻结）**：`risk′ = σ(logit(risk) + logit(π·q_H) − logit(m_train))`；等价实现 = θ 重映射 **`θ_eff(π) = σ(logit(θ) − logit(π·q_H) + logit(m_train))`**，θ=0.03 盈亏锚不动。实现 = `scripts/w4v3_make_armc.py` 产出 8 个 stophead JSON（v2 权重逐字节拷贝 + theta 重写 + armc 溯源块），**零 harness 改动**（`--delta-policy learned:v2` + `--stophead-model` 原机制）。
- **常量（从已归档产物算死，脚本 load 时复验）**：
  - **q_H = 731/748 = 0.97727**（`humdial_cuts.jsonl`：P(pause ≤ RISK_HORIZON 2.5 | 自发延续)——HumDial 对 C0 的唯一数值贡献，条件形态、非边际率）；
  - **m_train = 3307/19288 = 0.17145**（`ops_v2.jsonl`：P(修订于 horizon 内到来) 的 v2 训练边际；与 gen 收据 4744×69.7% 逐位自洽）；
...
$PY scripts/w4_ladder_report.py --arms w4v3c_pi020_tact w4v3c_pi040_tact \
    ...
    w4v3c_pi200_tact w4v3c_pi300_tact
```
