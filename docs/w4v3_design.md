...
> **状态：v1 FROZEN；Arm C0 Phase-2 π 网格已实跑，P1–P5/P4 裁决待回填（2026-07-15）**。结构/判据/探针门于实跑前冻结（第一段）；Phase-1 收据 = §9；数值常量与预测于 §10 冻结为 v1（第二段）；Phase-2 原始收据 = §10.6。Arm F/P 经探针判读不启用（§10.1，终局）。
