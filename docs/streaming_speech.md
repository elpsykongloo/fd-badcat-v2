...
生成过程中出错则显式 `speech_error` 并停止该回复；没有输出任何文本前保留 response 的一次超时重试与道歉回退，有部分输出后不重试，避免重复播报。TTS 失败不会伪造完成。未开启输入准入时历史仍使用完整生成文本；开启后对已取消回复只把确认播完的完整句子写入模型历史，打断状态留在内部记录和 trace，审计史保留全文；没有完整句子时不建立该轮 assistant 历史。仍不是词级或物理“听到前缀”对齐。

流式 TTS 已从提示词式朗读升级为 `verbatim-grammar-v2`：安全编码的 Thinker 单一原文字面量约束 + 文本全文/结束证明校验，防止 Omni 把待读疑问句重新回答；默认 stage0 固定 xgrammar，修复旧 choice 遇换行时自动回退导致核心崩溃的缺口。早到音频在校验前隔离（512KiB硬上限），没有证明不放行。需要启动器安装的窄范围服务适配，机制、旧整 WAV 兼容边界和音频回读验收见 [web_demo.md](web_demo.md) §8。
