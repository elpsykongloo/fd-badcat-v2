...
### 12.8 v2 transfer 曲线回填（服务器零 GPU，2026-07-14；本节覆盖 §12.7 的“待回填”占位）

`/root/miniconda3/envs/fd-sds/bin/python scripts/w4_ceiling_diag.py` exit 0。原始输出：

```text
transfer stophead_v0: AUC=0.679 W=1.5 frontier={0: 0.083, 1: 0.179, 2: 0.22, 3: 0.343, 4: 0.521}
transfer stophead_v1: AUC=0.644 W=1.5 frontier={0: 0.056, 1: 0.083, 2: 0.288, 3: 0.411, 4: 0.48}
transfer stophead_v2: AUC=0.752 W=1.5 frontier={0: 0.151, 1: 0.384, 2: 0.562, 3: 0.617, 4: 0.699} protect@theta=85.7%
```

JSON 精确值：v2 transfer AUC = **0.7520091848450058**，LOO AUC = **0.7531572904707233**，差 **0.0011481056257175**。因此按 AUC 口径，域随机化+特征收缩已把排序迁移从 v1 0.644 拉回到几乎追平域内 LOO；“迁移 AUC 仍 ~0.65”的失败分支被否定。但部署阈值下仍有 **85.7%** op 被保护，与实跑窗口保护 84.0% 同向，说明剩余主误差不是排序而是**风险绝对标度/校准迁移**。

容量判据 (i) 须严格按 §12.4 冻结口径而非只看 AUC：W=1.5、lost≤1 处 v2 回收 **38.4%**，LOO 下界约 **43.9%**，差约 **5.5 回收点**，以约 **0.5pt** 之差超过预注册的 `≤5pt` 硬线。故**条件 (i) 严格未成立（边缘）**；允许的论文表述是“AUC 层排序数据轴近乎榨干，但冻结经济前沿判据以 0.5pt 之差未过”，不能写成“容量条件(i)已立”。条件 (ii) 文本语义 LOO 探针未做且不被触发；不引入 0.6B、不重开 rung 4，ICASSP 分支与三代失败归因收口不变。完整机器可读收据已回写 `exp/w4/ceiling_diag.json`。
