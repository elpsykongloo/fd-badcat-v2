...
- **全部 GPU 项已跑完并逐文件核验**（用户跑数 7/07 晚，单线程复核 7/08）。汇报：**`手工文档/神谕/08_W3 执行汇报.md`**（07 缓发内容已并入）。要点：
  ...
  - **投机派发**：exact 56→57（+eco15 良性）；HumDial 门 off **63.38** / on **63.00** Δ−0.38 过（同日对称单判）；首响投影 1.141→**0.64 地板**（冻结档分解 recheck）。live 实测收益待 D6 标准卡串行复测。
  ...
  - HumDial 门产物已入库 `exp/w3/humdial_gate_spec{off,on}_summary.json`；`exp/w2_rerun/grid_full.json` 已被 v3 网格覆写（v2 数字重算核验一致；scorer 后续加组名防覆写）。
