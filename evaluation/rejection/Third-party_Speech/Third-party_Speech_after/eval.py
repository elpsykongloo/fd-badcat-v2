...
def eval_behavior_all(data_dir, client, task, output_dir):
    ...
    # 保存所有结果到一个JSON文件
    output_path = os.path.join(output_dir, "Third-party_Speech_after_content_tags.json") #不同子任务的输出结果
    with open(output_path, "w", encoding="utf-8") as f:
        for prefix, result in output_list:
            # 创建新的条目格式
            entry = {
                "key": f"Third-party_Speech_after_{prefix}", #不同子任务的key
...
if __name__ == "__main__":
    ...
    # 设置参数
    data_dir = "./dev/Third-party_Speech_after"  # 输入：存放wav音频数据的目录
    output_dir = "./dev/json_group"  # 输出：Third-party_Speech_after_content_tags.json文件的保存目录
